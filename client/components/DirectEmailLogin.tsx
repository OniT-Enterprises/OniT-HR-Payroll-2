...
export const DirectEmailLogin: React.FC = () => {
  ...
  const handleEmailSignIn = async () => {
    ...
    try {
      ...
      if (user) {
        setMessage('✅ Successfully signed in as ' + user.email + '! Setting up your workspace...');
        setAuthStatus(getAuthStatus());
        
        // Auto-setup tenant with the user's actual email
        const success = await autoSetupTenantForUser(user.uid, user.email || email);
        if (success) {
          setMessage('✅ Workspace created for ' + user.email + '! Refreshing page...');
          setTimeout(() => window.location.reload(), 2000);
        } else {
          setMessage('✅ Signed in as ' + user.email + ' but failed to create workspace. Try refreshing the page.');
        }
      }
    } catch (error: any) {
      ...
      if (error.code === 'auth/user-not-found') {
        userMessage = '❌ No account found for celestinod@gmail.com. Check Firebase Console Authentication.';
  ...
  return (
    <Card className="mb-6 border-blue-200 bg-blue-50">
      ...
      <CardContent className="space-y-4">
        <Alert>
          <AlertDescription>
            You're currently signed in as an anonymous user. Sign in with your actual email account: <strong>celestinod@gmail.com</strong>
          </AlertDescription>
        </Alert>
        ...
        {authStatus.isSignedIn && (
          <div className="bg-yellow-50 border border-yellow-200 p-3 rounded">
            <p className="text-sm text-yellow-800">
              <strong>Current user:</strong> {authStatus.email || `Anonymous (${authStatus.uid?.substring(0, 8)}...)`}
            </p>
            <Button 
