...
const moduleConfig = {
  hiring: {
    ...
    submenu: [
      ...
      { label: "Onboarding", icon: <UserPlus className="h-4 w-4" />, path: "/hiring/onboarding" },
      { label: "Offboarding", icon: <UserCog className="h-4 w-4" />, path: "/hiring/offboarding" },
    ],
  },
  staff: {
    id: "staff",
    ...
    submenu: [
      ...
      { label: "Organization Chart", icon: <Building2 className="h-4 w-4" />, path: "/staff/org-chart" },
    ],
  },
  timeleave: {
    id: "timeleave",
    ...
    submenu: [
      ...
      { label: "Shift Scheduling", icon: <Calendar className="h-4 w-4" />, path: "/time-leave/schedules" },
    ],
  },
  performance: {
    id: "performance",
    ...
    submenu: [
      ...
      { label: "Disciplinary", icon: <Shield className="h-4 w-4" />, path: "/performance/disciplinary" },
    ],
  },
  payroll: {
    id: "payroll",
    ...
    submenu: [
      ...
      { label: "Tax Reports", icon: <FileText className="h-4 w-4" />, path: "/payroll/tax-reports" },
      { label: "Benefits Enrollment", icon: <Heart className="h-4 w-4" />, path: "/payroll/benefits" },
      { label: "Deductions & Advances", icon: <DollarSign className="h-4 w-4" />, path: "/payroll/deductions" },
    ],
  },
  reports: {
    id: "reports",
    ...
    submenu: [
      ...
      { label: "Custom Reports", icon: <PieChart className="h-4 w-4" />, path: "/reports/custom" },
    ],
  },
};
...
export default function HotDogNavigation() {
  ...
  const getActiveModuleFromPath = () => {
    ...
    for (const [moduleId, config] of Object.entries(moduleConfig)) {
      if (path.startsWith(`/${moduleId}`) || config.submenu.some(item => path === item.path)) {
        return moduleId;
      }
    }
            ...
            <div className="flex space-x-1">
              {Object.entries(moduleConfig).map(([moduleId, config]) => (
                <Button
                  ...
                  `}
                >
                  <span className={activeModule === moduleId ? '' : config.color}>
                    {config.icon}
                  </span>
                  {config.label}
