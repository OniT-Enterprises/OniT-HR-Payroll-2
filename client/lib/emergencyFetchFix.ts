...
export const emergencyRestoreFetch = () => {
  ...
  const customProps = ['__originalFetch', '__originalXMLHttpRequest', '__originalFirebaseMethods'];
  ...
  const isCustom = fetchString.includes('Firebase') || fetchString.includes('blocked') || fetchString.includes('isolation');
