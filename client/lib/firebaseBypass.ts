...
export async function testDirectFirestoreAccess() {
  ...
  try {
    ...
    const employeeSnapshot = await getDocs(
      query(employeeCollection, orderBy("createdAt", "desc")),
    );
    ...
    const departmentSnapshot = await getDocs(
      query(departmentCollection, orderBy("name", "asc")),
    );
    const departmentCount = departmentSnapshot.docs.length;

    console.log(
      `✅ Direct access to departments: ${departmentCount} documents`,
    );
    ...
    if (error.code === "permission-denied") {
      return {
        ...
        message:
          "Firestore rules require authentication. Please enable Anonymous Auth in Firebase Console.",
      };
    }
