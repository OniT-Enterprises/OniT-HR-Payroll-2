...
export const paths = {
  ...
  disciplineAction: (tid: string, actionId: string) => `tenants/${tid}/discipline/${actionId}`,
  ...
  payrunInputs: (tid: string, yyyymm: string) => `tenants/${tid}/payrunInputs/${yyyymm}`,
  payrunInput: (tid: string, yyyymm: string, empId: string) => `tenants/${tid}/payrunInputs/${yyyymm}/${empId}`,
...
export const isValidTenantId = (tid: string): boolean => {
  return typeof tid === 'string' && tid.length > 0 && /^[a-zA-Z0-9_-]+$/.test(tid);
};
