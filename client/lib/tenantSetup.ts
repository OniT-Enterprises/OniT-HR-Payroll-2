 ...
 */
export const createDefaultTenant = async (config: TenantSetupConfig): Promise<boolean> => {
  if (!db) {
  ...
  try {
    ...
    await setDoc(doc(db, paths.member(config.tenantId, config.userId)), memberData);
    console.log('✅ Member document created');

    // 3. Create some sample departments
    const departments = [
      { name: 'Engineering', description: 'Software development team', headCount: 0 },
      { name: 'Human Resources', description: 'HR and people operations', headCount: 0 },
      { name: 'Marketing', description: 'Marketing and communications', headCount: 0 },
    ];
...
export const generateTenantId = (userEmail: string): string => {
  ...
  return `tenant-${emailPrefix}-${timestamp}`.toLowerCase().replace(/[^a-z0-9-]/g, '');
};
 ...
 */
export const autoSetupTenantForUser = async (userId: string, userEmail: string): Promise<boolean> => {
  try {
    ...
    if (success) {
      console.log('✅ Auto-setup completed. User should now have tenant access.');
