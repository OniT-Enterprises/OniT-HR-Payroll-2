...
class LocalDataService {
  ...
  async addEmployee(employeeData: Omit<Employee, 'id' | 'createdAt' | 'updatedAt'>): Promise<string> {
    ...
    console.log('✅ Employee added:', employee.personalInfo.firstName, employee.personalInfo.lastName);
  ...
  async addDepartment(departmentData: Omit<Department, 'id' | 'createdAt' | 'updatedAt'>): Promise<string> {
  ...
  async addPosition(positionData: Omit<Position, 'id' | 'createdAt' | 'updatedAt'>): Promise<string> {
  ...
  private initializeSampleData(): void {
    ...
    if (this.departments.length === 0) {
      ...
      const sampleDepartments = [
        { name: 'Engineering', description: 'Software development and technical operations', headCount: 12 },
        { name: 'Human Resources', description: 'People operations and talent management', headCount: 3 },
        { name: 'Marketing', description: 'Brand, marketing, and communications', headCount: 5 },
        { name: 'Sales', description: 'Business development and customer relations', headCount: 8 },
        { name: 'Finance', description: 'Accounting, finance, and business operations', headCount: 4 }
      ];
      ...
      const samplePositions = [
        { title: 'Senior Software Engineer', department: 'Engineering', description: 'Full-stack development', requirements: ['React', 'Node.js', '5+ years exp'], isOpen: true },
        { title: 'Product Manager', department: 'Engineering', description: 'Product strategy and roadmap', requirements: ['Product management', 'Technical background'], isOpen: true },
        { title: 'HR Coordinator', department: 'Human Resources', description: 'Recruitment and employee relations', requirements: ['HR experience', 'Communication skills'], isOpen: false },
        { title: 'Marketing Specialist', department: 'Marketing', description: 'Digital marketing and content creation', requirements: ['Marketing degree', 'Social media experience'], isOpen: true }
      ];
